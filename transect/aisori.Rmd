...
# save(result, aisoriStattion.df, file =  paste0(mm_path, "/cli/aisori/","aisori.Rdata"))
```
#### 
...
linearTrends <- as.data.frame(matrix(0, ncol = 17, nrow = 11, byrow = TRUE))
linearTrends <- setNames(linearTrends, colNames.b)

for (i in 1:11) {
    sti <- as.data.frame(result1[i])
    code <- sti[1, 1]  # code
    linearTrends[i, 1] <- code
