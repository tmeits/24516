...
eval16CliPars <- function(stationCode, data.cli, data.calc, beginYear, endYear) {
    ...
    lastCol <- c("STDAT0", "STDAT5", "FDAT0", "FDAT5", "INTER0", "INTER5", "MAXT", "MDAT", "SUMT0", 
    ...
    for (i in 1:l) {
        cat("****************Year=", period[i], "Observation=",numberObservation(data.cli, period[i]),"**********************\n")
        df16$Station_Code[i] <- stationCode
        df16$Year[i] <- period[i]
        # There is the initial year of observation is finite. The intermediate may be missing. Skip
        # the calculation cycle.
        if (numberObservation(data.cli, period[i]) > 69) {
