...
readClimateMinusinsk <- function(url) {
    ...
    # 53.42N 91.42E
    # http://www.khti.ru/documents/%D0%9D%D0%9E%D0%9B/%D0%9F%D1%83%D0%B1%D0%BB%D0%B8%D0%BA%D0%B0%D1%86%D0%B8%D0%B8/2014_2_En.pdf
...
# Считать начало сезона роста переход суммарной температуры sumT за 10 последовательных дней через пороговое значение в 90 градусов Цельсия. Тогда началом сезона роста можно считать первый день в этой последовательности, когда это условия будет выполнено (т.е. sumT>=90). 
...
seasonBG(Minusinsk.cli, 1961) # test
...
write_eval_clipars(paste0(mm_path, '/e16cp.csv'), E)
...
require(openxlsx)
# test data.frame's
testWriteReadXLSX <- function(fileName) {
    l <- list("iris" = iris, "mtcars" = mtcars, chickwts = chickwts, quakes = quakes)
    openxlsx::write.xlsx(l, file = fileName)
    R <- openxlsx::read.xlsx(xlsxFile = fileName, sheet = 3, skipEmptyRows = TRUE)
    return(R)
}
WR <- testWriteXLSX(paste0(mm_path, "/datasets.xlsx"))
